...
- 🐛 إصلاح خلط ترتيب النص في "سجل الأحداث" لما تختلط عربي/إنجليزي (أسماء أجهزة، أكواد إيرور، أسهم) — بيستخدم دلوقتي Unicode Bidi Isolates بدل تشغيل bidi على السطر كله دفعة واحدة
- 📋 زرار نسخ سجل الأحداث كامل للـ clipboard
- 🛑 إمكانية إلغاء عملية التفريغ/التلخيص وهي شغالة
- ⏸ إيقاف مؤقت للتسجيل (استراحة) من غير قفل الجزء الحالي
- 💾 تحذير قبل بدء التسجيل لو مساحة الديسك قليلة
- 🧮 تقدير تقريبي لحجم التوكينز قبل الموافقة على المعالجة
- 💬 رسائل خطأ مبسّطة لأشهر أخطاء الـ API (مفتاح غلط، تجاوز الحد المسموح، مشاكل اتصال)
- 🔒 تأكيد إضافي (كتابة اسم المحاضرة) قبل أي مسح نهائي
- 🔄 زرار تحديث + بحث داخل نافذة عرض النوتس
- ⚠️ تحذير عند إغلاق البرنامج أثناء عملية تفريغ/تلخيص شغالة (مش بس أثناء التسجيل)
